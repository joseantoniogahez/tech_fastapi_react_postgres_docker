...
| `/admin/audit-log`   | `protected`   | `features/audit-log/AuditLogPage`                 | Requires `audit_logs:read`; missing permission redirects to `/welcome`.                                                                                          |
...
| `/admin/users`       | `protected`   | `features/admin-users/AdminUsersPage`             | Requires `users:manage`; missing permission redirects to `/welcome`, while role-related controls stay hidden in-page unless the session also has `roles:manage`. |
