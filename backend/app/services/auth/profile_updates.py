...
class AuthProfileUpdates:
    ...
    async def build_username_change(self, current_user: User, username: str | None) -> tuple[str, dict[str, str]]:
        if username is None:
    ...
    async def persist_user_changes(self, current_user: User, changes: dict[str, str]) -> User:
        return await self._auth_repository.update(current_user, **changes)
