...
| `GET`    | `/books/`                                           | No   | No                        | Optional query: `author_id` (`>=1`), `offset` (`>=0`), `limit` (`1..100`), `sort` in `{id, -id, title, -title, year, -year}` | `200` `BookResponse[]`             | `400`, `500`                             |
...
- `GET /books/` supports optional `author_id` filtering, pagination (`offset`/`limit`), and deterministic sorting.
- `GET /books/` defaults to sorting by `id` ascending.
